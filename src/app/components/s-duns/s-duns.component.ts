...
@Component({
  selector: 'app-s-duns',
  templateUrl: './s-duns.component.html',
  styleUrls: ['./s-duns.component.css']
})
export class SDunsComponent implements OnInit {
  ...
  get fval() {
    ...
    public updateCard(){
      this._uw.card.services[3].status="proccess"; 
      this.dataApiService.updateCard(this._uw.card,this._uw.card.id).subscribe();
        }
    public proccess (){
      this._uw.quote.serviceDescription="Registro DUNS";
      this._uw.quote.amount=this._uw.amount;
      this._uw.quote.flag="svv";
      this._uw.quote.status="proccess";
    this._uw.quote.message="nueva solicitud DUNS";
		this._uw.quote.subject="nuevo DUNS";
		this._uw.quote.messageSubject="nuevo DUNS";
    ...
    public getInfo(){
      ...
      .subscribe((info: InfoInterface) => {
        ...
        this._uw.amount=this.info[0].prices[6].price;
           } );
